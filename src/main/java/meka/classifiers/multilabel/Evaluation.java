...
import meka.classifiers.multilabel.meta.MLFilteredClassifier;
...
/**
 * Evaluation.java - Evaluation functionality.<br>
 * Modified by Álvar Arnaiz-González (July 18, 2016) for adding a new result: compression.
 * 
 * @author 		Jesse Read
...
public class Evaluation {
	...
	public static Result cvModel(MultiLabelClassifier h, Instances D, int numFolds, String top, String vop) throws Exception {
		...
		// START-AAG 20160718 New metric: compression
		else if (h instanceof MLFilteredClassifier) {
			r.setValue("Compression", ((MLFilteredClassifier)h).getCompression());
			r.setValue("Filtering time", ((MLFilteredClassifier)h).getFilteringTime());
		}
		// END-AAG
		r.setInfo("Verbosity",vop);
		r.output = Result.getStats(r, vop);
	...
	public static Result evaluateModel(MultiLabelClassifier h, Instances D_train, Instances D_test) throws Exception {
		...
		// START-AAG 20160718 New metric: compression
		if (h instanceof MLFilteredClassifier) {
			result.setValue("Compression", ((MLFilteredClassifier)h).getCompression());
			result.setValue("Filtering time", ((MLFilteredClassifier)h).getFilteringTime());
		}
		// END-AAG
